...
#
# Crawled sites:
#   https://developer.ui.com         — Ubiquiti developer portal (API reference)
#   https://help.ui.com              — UniFi support docs
#   https://modelcontextprotocol.io  — MCP protocol docs
#
# Repomix packs:
#   Art-of-WiFi/unifios-utilities    — Community UniFi API docs / examples
#   modelcontextprotocol/rust-sdk    — rmcp Rust SDK
...
while [[ $# -gt 0 ]]; do case "$1" in
  ...
  --skip-repomix) SKIP_REPOMIX=true;shift ;; -h|--help)
    echo "Usage: scripts/refresh-docs.sh [--dry-run] [--skip-crawl] [--skip-repomix]"; exit 0 ;;
...
| \`unifi/repos/\`  | Community UniFi API utilities (repomix) | Art-of-WiFi/* |
| \`mcp/docs/\`    | MCP protocol docs | modelcontextprotocol.io |
| \`mcp/repos/\`   | MCP Rust SDK (repomix) | modelcontextprotocol/* |
## Note on TLS
UniFi controllers use self-signed certs. UNIFI_SKIP_TLS_VERIFY=true is normal.
UDM path prefix: /proxy/network — legacy controllers omit it.
...
main() {
  ...
  if [[ "$SKIP_REPOMIX" != true ]]; then
    ...
    pack_repo "Art-of-WiFi/UniFi-API-client"       "unifi/repos/art-of-wifi-unifi-api-client.xml" "src/**" ""
    # MCP Rust SDK
