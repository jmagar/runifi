...
- `hybrid`: convenience actions that prefer official API and can use internal actions when requested.
...
The internal reference inventory is registry-backed so all 180 target actions are discoverable by MCP clients. Existing and explicitly mapped internal actions use known controller endpoints; the broader inventory remains a compatibility surface until each upstream path is verified against live controller behavior.
